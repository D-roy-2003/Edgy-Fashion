...
3. **Set up environment variables**
   ...
   **Important**: All environment variables are required for the application to function. The `.env.local` file is excluded from version control for security purposes.
...
- **Note**: Admin credentials are set up via the seed script. Create your own admin account using the seed script after setting up your environment variables.
...
# JWT Secret for Authentication (use a strong, randomly generated secret)
...
**Security Note**: Never commit environment variables or credentials to version control. All sensitive configuration is handled through environment variables and is excluded from the repository via `.gitignore`.
